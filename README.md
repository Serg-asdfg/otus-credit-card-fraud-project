...
Выполнение плана по ДЗ2:
3.b) Развернуть требуемую дев среду в облаке Яндекса - сделано, через Terraform
3.с) Загрузить входные данные в форме csv - данные загружены в формате txt, общим объёмом 112,31ГБ, 40 файлов,
    публичный бакет внутри Yandex Cloud: otus-bucket-b1g46o5r67v2nlgqhi5s. Проверка загрузки данных в HDFS кластера
    dataproc в YC - выполнено.
    Снимки с экрана прилагаются:
    ![alt text](<Схема сети ДЗ2.png>)
    ![alt text](<S3 storage content.png>)
    ![alt text](<S3 storage content(2).png>)
    ![alt text](<HDFS content.png>)